...
export async function GET(request: NextRequest) {
  ...
  const adGroupId  = searchParams.get('ad_group_id')   ?? ''   // optional — filters to one ad group
  ...
  if (adGroupId && !CAMPAIGN_ID_RE.test(adGroupId))
  ...
  try {
    const ads = await getAds(clientId, campaignId, startDate, endDate, adGroupId || undefined)
