...
import { getDevicePerformance } from '@/lib/google-ads'
...
export async function GET(request: NextRequest) {
  ...
  try {
    const rows = await getDevicePerformance(clientId, startDate, endDate, campaignId || undefined)
    return NextResponse.json({ rows })
  } catch (err: any) {
    console.error('[device-performance]', err?.message ?? err)
    return NextResponse.json({ error: err.message ?? 'Failed to load device performance' }, { status: 500 })
  }
}
