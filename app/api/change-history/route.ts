...
import { getChangeHistory } from '@/lib/google-ads'
...
export async function GET(request: NextRequest) {
  ...
  try {
    const events = await getChangeHistory(clientId, startDate, endDate, campaignId || undefined)
    return NextResponse.json({ events })
  } catch (err: any) {
    console.error('[change-history]', err?.message ?? err)
    return NextResponse.json({ error: err.message ?? 'Failed to load change history' }, { status: 500 })
  }
}
