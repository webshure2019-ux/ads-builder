...
export async function GET(request: NextRequest) {
  ...
  try {
    const keywords = await getKeywords(clientId, startDate, endDate, campaignId || undefined)
    return NextResponse.json({ keywords })
  } catch (err: any) {
    console.error('[keyword-performance] Google Ads API error:', err?.message ?? err)
