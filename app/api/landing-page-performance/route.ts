...
export async function GET(request: NextRequest) {
  ...
  try {
    const rows = await getLandingPagePerformance(clientId, startDate, endDate, campaignId || undefined)
    return NextResponse.json({ rows })
  } catch (err: any) {
    console.error('[landing-page-performance]', err?.message ?? err)
    return NextResponse.json({ error: err.message ?? 'Failed to load landing page performance' }, { status: 500 })
  }
}
