...
export async function GET(request: NextRequest) {
  ...
  try {
    const terms = await getSearchTerms(clientId, startDate, endDate, campaignId || undefined)
    return NextResponse.json({ terms })
  } catch (err: any) {
    console.error('[search-terms] Google Ads API error:', err?.message ?? err)
    return NextResponse.json({ error: err.message ?? 'Failed to load search terms' }, { status: 500 })
  }
}
