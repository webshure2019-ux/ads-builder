...
const FEATURES: { emoji: string; text: string; date: string }[] = [
  { emoji: '◧', text: 'Expandable section rail — click the chevron at the top of the left rail to expand it from 56 px icon-only to 208 px with section labels, group headers (Overview / AI / Spend / Health / Movers & Search), and inline keyboard-shortcut hints. State persists across reloads.', date: '13 May 2026' },
...
const FIXES: { text: string; date: string }[] = [
  { text: '🔒 Security hardening — (1) /api/auth is now rate-limited (5 attempts / 15 min per IP, then a 15 min block) with a fixed per-attempt delay; the single shared password could previously be brute-forced unthrottled. (2) Password and session-token comparisons are now constant-time (timing-attack resistant) in the login route, requireAuth, and middleware. (3) The landing-page scraper SSRF guard now resolves DNS and validates every resolved IP, normalises numeric IP encodings (decimal/hex/octal), and re-validates the address at socket-connect time — closing DNS-to-private-IP and DNS-rebinding bypasses.', date: '13 May 2026' },
  { text: 'Fixed blank Landing Pages section — three issues stacked: the lib function silently swallowed GAQL errors with `.catch(() => [])` so failures were indistinguishable from "no data"; the component returned `null` when rows was empty so the panel showed nothing at all; the loading skeleton used hardcoded white. Now: errors propagate with a themed retry message; empty state renders an informative card explaining `landing_page_view` lag (24-48h) and PMax exclusion; loading skeleton themes correctly. Also filter widened from `clicks > 0` to `clicks || impressions || cost` so fresh URLs with impressions but no clicks yet are visible. Added defensive campaign.id-in-SELECT when used in WHERE.', date: '13 May 2026' },
  { text: 'Fixed Quality Score Tracker missing-table error — when the Supabase `keyword_qs_snapshots` table hadn\'t been created, the section showed a raw "Could not find the table" error. Now the API detects the schema-cache miss and the panel renders a friendly one-time setup card with the CREATE TABLE SQL and a Copy button, so users can paste it into the Supabase SQL editor without leaving the app.', date: '13 May 2026' },
